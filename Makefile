# ado-mcp — build & quality targets

BINARY      := ado
PKG         := ./cmd/ado
VERSION     ?= $(shell git describe --tags --always --dirty 2>/dev/null || echo dev)
LDFLAGS     := -ldflags "-X github.com/rangertaha/ado-mcp/internal.version=$(VERSION)"
...
## run: build and run (expects ADO_ORG_URL and ADO_PAT in the environment)
run: build
	./bin/$(BINARY)
